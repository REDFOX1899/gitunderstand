...
export function ArchitectureSection() {
  const { summaries, generateSummary, aiAvailable } = useWikiContext();

  const summary = summaries.architecture;

  return (
    <div>
      <h1 className="mb-1 text-2xl font-bold text-stone-900">Architecture</h1>
      <p className="mb-6 text-sm text-stone-400">
        System design, patterns, and component relationships
      </p>
      ...
      {!summary?.content && !summary?.loading && !summary?.error && (
        <div className="text-center">
          {aiAvailable ? (
            <button
              onClick={() => void generateSummary("architecture")}
