...
from core.parser import parse_local_dir_path, parse_remote_repo
from core.utils.auth import resolve_token
from core.utils.ignore_patterns import load_ignore_patterns
from core.utils.pattern_utils import process_patterns
from core.utils.query_parser_utils import KNOWN_GIT_HOSTS
...
async def ingest_async(
    ...
    *,
    ...
    # Python 3.11+ native str.removesuffix replaces compat_func.removesuffix
    source = source.strip().removesuffix(".git")
...
async def _clone_repo_if_remote(query: IngestionQuery, *, token: str | None) -> AsyncGenerator[None]:
    ...
    """
    # Python 3.12+ uses onexc; earlier uses onerror. Since we target 3.11+, handle both.
    kwargs = {}
